...
public class RBoardDAO {
	...
	private RBoardDTO makeArticleFromResult() throws Exception {
		...
		article.setR_category(rs.getInt("r_category")); //글카테고리->11:해리포터1, 12:해리포터2, 13:해리포터3,,,
		article.setR_ref(rs.getInt("r_ref")); //게시판분류->1:자유, 2:정보, 3:별점, 4:hot
	...
	public int getArticleSearchCount(String search, String searchtext) {
		...
		try {
			...
			if (search==null || search=="") { //검색분야를 선택X
				...
				if (search.equals("r_title_r_body")) { //제목+본문
	...
	public int getArticleSearchCount(String search, String searchtext, String r_ref) {
		...
		try {
			...
			System.out.println("getArticleSearchCount()r_ref 검색어=>" + sql);
			// --------------------------------------------------------------------------
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			...
			System.out.println("getArticleSearchCount()r_ref 에러 발생=>" + e);
		} finally {
			pool.freeConnection(con, pstmt, rs);
		}
	...
	//글목록조회-검색결과, r_ref 포함
		public List<RBoardDTO> getBoardArticles(int start, int end, String search, String searchtext, String r_ref) {
			...
			try {
				...
				System.out.println("getBoardArticles()r_ref 의 sql=>"+sql);
				//-------------------------------------------------------
				pstmt=con.prepareStatement(sql);
				pstmt.setInt(1, start); //mysql은 start-1
				pstmt.setInt(2, end); //end-1??
				...
				System.out.println("getBoardArticles()r_ref 에러 발생=>"+e);
			}finally {
				pool.freeConnection(con, pstmt, rs);
			}
	...
	public void insertArticle(RBoardDTO article) {
		...
		System.out.println("insertArticle()메서드 내부 r_number=>" + r_number);
