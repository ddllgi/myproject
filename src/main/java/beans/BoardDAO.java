...
public class BoardDAO {
	...
	private BoardDTO makeHArticleFromResult() throws Exception {
		...
		article.setCategory(rs.getInt("h_category")); // 글카테고리->11:해리포터1, 12:해리포터2, 13:해리포터3,,,
		article.setRef(rs.getInt("h_ref")); // 게시판분류->1:자유, 2:정보, 3:별점, 4:hot
	...
	private BoardDTO makeZArticleFromResult() throws Exception {
		...
		article.setCategory(rs.getInt("z_category")); // 글카테고리->11:해리포터1, 12:해리포터2, 13:해리포터3,,,
		article.setRef(rs.getInt("z_ref")); // 게시판분류->1:자유, 2:정보, 3:별점, 4:hot
	...
	private BoardDTO makeMArticleFromResult() throws Exception {
		...
		article.setCategory(rs.getInt("m_category")); // 글카테고리->11:해리포터1, 12:해리포터2, 13:해리포터3,,,
		article.setRef(rs.getInt("m_ref")); // 게시판분류->1:자유, 2:정보, 3:별점, 4:hot
	...
	private BoardDTO makeGArticleFromResult() throws Exception {
		...
		article.setCategory(rs.getInt("g_category")); // 글카테고리->11:해리포터1, 12:해리포터2, 13:해리포터3,,,
		article.setRef(rs.getInt("g_ref")); // 게시판분류->1:자유, 2:정보, 3:별점, 4:hot
	...
	private BoardDTO makeRArticleFromResult() throws Exception {
		...
		article.setCategory(rs.getInt("r_category")); // 글카테고리->11:해리포터1, 12:해리포터2, 13:해리포터3,,,
		article.setRef(rs.getInt("r_ref")); // 게시판분류->1:자유, 2:정보, 3:별점, 4:hot
	...
	public void scrap(BoardDTO article) {
		...
		//담을 정보:스크랩 순번, 아이디, 스크랩한 글의 원래 번호(h_number~), 스크랩한 글의 카테고리, 스크랩한 글의 제목, 스크랩 날짜
		...
		try {
			...
			if (scrap > 0) {
				...
				con.commit();
				System.out.println("스크랩수 증가여부(update)=>"+update+" //(1성공 0실패)");
			}
		}catch (Exception e) {
	...
	public void rate(BoardDTO article) {
		...
		int number=0;
		System.out.println("rank() 메서드 내부 v_number=>"+v_number);
		...
		try {
			...
			rs=pstmt.executeQuery();
			if (rs.next()) { //데이터가 있으면->수정
				//수정할 데이터 찾는 메서드 밑에 작성하기
				...
				int update=pstmt.executeUpdate();
				con.commit();
				System.out.println("별점 or 좋아요 수정 성공여부(update)=>"+update);
				
				//원래의 수정 메서드에는 반환값이 있음... 어떻게 할지?
			}else { //데이터가 없으면->새로 입력
				...
				if (rs.next()) {
					number=rs.getInt(1)+1;
					System.out.println("별점 작성 번호(number)=>"+number);
				}else {
					number=1;
				}
				con.commit();
				...
				con.commit();
				System.out.println("별점 or 좋아요 등록 성공여부(insert)=>"+insert);
			}
