...
public class HBoardDAO {
	...
	private HBoardDTO makeArticleFromResult() throws Exception {
		...
		article.setH_category(rs.getInt("h_category")); // 글카테고리->11:해리포터1, 12:해리포터2, 13:해리포터3,,,
		article.setH_ref(rs.getInt("h_ref")); // 게시판분류->1:자유, 2:정보, 3:별점, 4:hot
	...
	public int getArticleSearchCount(String search, String searchtext) {
		...
		try {
			...
			if (search == null || search == "") { // 검색분야를 선택X
				...
				if (search.equals("h_title_h_body")) { // 제목+본문
